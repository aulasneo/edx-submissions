...
class TestXqueueViewSet(APITestCase):
    ...
    def test_dispatch_no_session_attribute(self):
        """Test dispatch when request has no session attribute"""

        class RequestWithoutSession:
            def __init__(self, original_request):
                for attr in dir(original_request):
                    if attr != 'session' and not attr.startswith('_'):
                        setattr(self, attr, getattr(original_request, attr))

        factory = APIRequestFactory()
        base_request = factory.get(self.url_status)
        request = RequestWithoutSession(base_request)

        view = XqueueViewSet.as_view({'get': 'status'})
        force_authenticate(request, user=self.user)

        with patch('rest_framework.viewsets.ViewSet.dispatch') as mock_dispatch:
            mock_dispatch.return_value = HttpResponse()
            response = view(request)

            mock_dispatch.assert_called_once_with(request)

            self.assertEqual(response, mock_dispatch.return_value)

    def test_dispatch_malformed_cookie(self):
        """Test dispatch with malformed cookie that doesn't contain '='"""
        factory = APIRequestFactory()
        request = factory.get(self.url_status)
        request.headers = {'Cookie': 'malformedcookie'}  # Cookie without '='
    ...
    def test_dispatch_session_with_nonexistent_user(self):
        """Test dispatch when session contains auth data for a non-existent user"""
        factory = APIRequestFactory()
        request = factory.get(self.url_status)

        # Create a session with a non-existent user ID
        session = SessionStore()
        session['_auth_user_id'] = 99999  # Non-existent user ID
        session.save()

        # Add session cookie to request
        request.headers = {'Cookie': f'sessionid={session.session_key}'}
        request.session = session

        view = XqueueViewSet.as_view({'get': 'status'})

        with patch('rest_framework.viewsets.ViewSet.dispatch') as mock_dispatch, \
                patch('submissions.views.xqueue.log') as mock_log:
            mock_dispatch.return_value = HttpResponse()
            response = view(request)

            mock_dispatch.assert_called_once_with(request)
            mock_log.warning.assert_called_with("User not found for session")
            self.assertEqual(response, mock_dispatch.return_value)

    def test_dispatch_empty_cookie_header(self):
        """Test dispatch with empty Cookie header"""
        factory = APIRequestFactory()
        request = factory.get(self.url_status)
        request.headers = {'Cookie': ''}  # Empty cookie header
    ...
    def test_dispatch_invalid_session_expiry(self):
        """Test dispatch when session has expired"""
        factory = APIRequestFactory()
        request = factory.get(self.url_status)

        # Create a session and set it to expired
        session = SessionStore()
        session.set_expiry(-1)  # Set expiry to a past time
        session.save()
        ...
        with patch('rest_framework.viewsets.ViewSet.dispatch') as mock_dispatch, \
            ...
            mock_dispatch.assert_called_once_with(request)
            mock_log.warning.assert_called_with("Expired session")
            self.assertEqual(response, mock_dispatch.return_value)

    def test_dispatch_session_exists_but_invalid(self):
        """Test dispatch when session exists but is invalid when trying to load"""
        factory = APIRequestFactory()
        request = factory.get(self.url_status)

        session_key = "=" + ('a' * 32)

        request.headers = {'Cookie': f'sessionid{session_key}'}
        request.session = SessionStore()

        view = XqueueViewSet.as_view({'get': 'status'})

        with patch.object(SessionStore, 'exists') as mock_exists, \
                patch('rest_framework.viewsets.ViewSet.dispatch') as mock_dispatch:
    ...
    def test_parse_cookies(self):
        request = HttpRequest()
        request.headers = {
            'Cookie': 'sessionid=valid_session_id; other_cookie=value'
        }
        view = XqueueViewSet

        cookies = view.parse_cookies(request)
        expected_cookies = {
    ...
    def test_parse_cookies_no_cookies(self):
        request = HttpRequest()
        request.headers = {
            'Cookie': ''
        }
    ...
    def test_parse_cookies_invalid_format(self):
        request = HttpRequest()
        request.headers = {
            'Cookie': 'sessionid=valid_session_id; invalid_cookie_format'
        }
